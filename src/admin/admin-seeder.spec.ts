...
describe('admin-seeder', () => {
  ...
  it('throws when password is missing letters/numbers mix', () => {
    expect(() =>
      parseAdminSeedInput({
        ADMIN_PHONE: '+201000000000',
        ADMIN_PASSWORD: 'onlyletters',
      }),
    ).toThrow('ADMIN_PASSWORD must contain letters and numbers');
  });
  ...
  it('creates missing admin and returns created=true', async () => {
    ...
    const result = await seedAdminUser({ query } as any, { phone: '+201000000000' }, 'hash');
